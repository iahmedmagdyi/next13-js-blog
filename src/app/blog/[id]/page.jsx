...
const BlogDetails = (ctx) => {
    ...
    useEffect(() => {
        async function fetchComments() {
            const res = await fetch(`http://127.0.0.1:3000/api/comment/${ctx.params.id}`, { cache: 'no-store' })
    ...
    useEffect(() => {
        async function fetchBlog() {
            const res = await fetch(`http://127.0.0.1:3000/api/blog/${ctx.params.id}`, { cache: 'no-store' })
    ...
    const handleDelete = async () => {
        try {
            ...
            if (confirmModal) {
                const res = await fetch(`http://127.0.0.1:3000/api/blog/${ctx.params.id}`, {
                    headers: {
    ...
    const handleLike = async () => {
        try {
            const res = await fetch(`http://127.0.0.1:3000/api/blog/${ctx.params.id}/like`, {
                headers: {
                    'Authorization': `Bearer ${session?.user?.accessToken}`
                },
                method: 'PUT'
    ...
    const handleComment = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://127.0.0.1:3000/api/comment`, {
                headers: {
